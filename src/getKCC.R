#' @author Divya Mistry
#' @description Function to get kendall's tau correlation based on DIP interactors
#' 
...
#' 
message("NOTE: getKCC() requires expression matrix in the current environment")

# nodeApsets<-V(connGraph)[287]$Affy.PSs; nodeBpsets<-V(connGraph)[288]$Affy.PSs
getKCC <- function(nodeApsets, nodeBpsets, min = FALSE) {
  ...
  if(is.na(nodeApsets) || is.na(nodeBpsets)) {
    ...
    cor(x = expr.rma[splitA,], y = expr.rma[splitB,], method = "kendall")
  }
}