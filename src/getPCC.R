#' @author Divya Mistry
#' @description Function to get pearson correlation based on DIP interactors
#' 
...
#' 
message("NOTE: getPCC() requires expression matrix in the current environment.")

# nodeApsets<-V(connGraph)[287]$Affy.PSs; nodeBpsets<-V(connGraph)[288]$Affy.PSs
getPCC <- function(nodeApsets, nodeBpsets, min = FALSE) {
#   # if expression matrix is missing, stop moving forward
#   if (!"expr.rma" %in% ls()) {
#     stop("No expr.rma found. Set a global expression matrix.")
#   } else {
#     print("found expr.rma")
#   }
  ...
  if(is.na(nodeApsets) || is.na(nodeBpsets)) {
    ...
    cor(x = expr.rma[splitA,], y = expr.rma[splitB,], method = "pearson")
  }
}