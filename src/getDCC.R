#' @author Divya Mistry
#' @description Function to get distance correlation based on DIP interactors
#' 
#' @param nodeApsets string, probesets for one of the incident nodes for edge of interest
#' @param nodeBpsets string, probesets for the other incident node to edge of interest
...
#' @param index number in (0.0, 2.0] range. This index is the exponent of euclidean distance in dcor() as defined
#'              in energy package.
#' 
#' @details Requires the expression data loaded in current environment
#' @return pairwise distance correlation value for most abundant probesets corresponding to the DIP interactors represted by nodeA--nodeB edge.
#' 
message("NOTE: getDCC() requires expression matrix in the current environment")

# nodeApsets<-V(connGraph)[287]$Affy.PSs; nodeBpsets<-V(connGraph)[288]$Affy.PSs
getDCC <- function(nodeApsets, nodeBpsets, min = FALSE, index = 1) {
  require(energy) #for dcor()
  ...
  if(is.na(nodeApsets) || is.na(nodeBpsets)) {
    ...
    dcor(x = expr.rma[splitA,], y = expr.rma[splitB,], index = index)
  }
}