#' @author Divya Mistry
#' @description This file is used to read in GSE3076 expression CEL files, normalize the data
...
gse3076 <- ReadAffy(celfile.path = "data/GSE3076/")
rma.gse <- rma(gse3076)
...
#' keep just the wild-type chips
expr.rma <- expr.rma[,49:96]

#' use median of the biological replicate to represent expression level of gene at given time
numEvents <- 16 # number of time points
numReps <- 3 # number of replicates for each time point for a gene
repOneIdxs <- seq(from = 1, to = 48, by = numReps) # indices for replicate 1
repTwoIdxs <- seq(from = 2, to = 48, by = numReps) # indices for replicate 2
repThreeIdxs <- seq(from = 3, to = 48, by = numReps) # indices for replicate 3
median.expr.rma <- matrix(nrow = dim(expr.rma)[1], ncol = numEvents) # matrix to store median'd result
rownames(median.expr.rma) <- rownames(expr.rma)
colnames(median.expr.rma) <- paste(1:numEvents, "h", sep = "")
for(i in 1:dim(expr.rma)[1]) {
  for(j in 1:numEvents) {
    median.expr.rma[i, j] <- median(expr.rma[i, c(repOneIdxs[j], repTwoIdxs[j], repThreeIdxs[j])])
  }
}

#' save the processed expression
expr.rma <- median.expr.rma # expression data is used in older code, where expr.rma name is expected.
saveRDS(object = expr.rma, file = "data/Full_RMA_Processed_GSE3076_Expression_Matrix.RDS", compress = TRUE)
#' remove unnecessary objects
rm(median.expr.rma, numEvents, numReps, repOneIdxs, repTwoIdxs, repThreeIdxs)
...
saveRDS(object = keeperPS, file = "data/gse3076_uniqueProbeSets_represented_by_NetAffx.RDS", ascii = T)

#' remove unnecessary objects
rm(expr.rma, rma.gse, keeperPS, transcribedRegions, YG_S98Annot)